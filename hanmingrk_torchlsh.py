  ...
  def batch_query(self, xb, query_points, k=None):
    ...
    for hash_val, query_indices in query_buckets.items():
        ...
        if len(bucket_indices) < max_bucket_size:
          ...
          sorted_keys = sorted(keys, key=lambda key: self.calculate_distance_key(hash_val, key, "hamming"))
          curkey = 0
          if sorted_keys[0] == hash_val:
              curkey = 1
          while len(bucket_indices) < max_bucket_size:
            bucket_indices.extend(self.hashtables.get(sorted_keys[curkey], []))
            curkey += 1
